/* global AFRAME THREE identityMatrix matrixCopy boundingBoxOf vector3d */
var chai = chai || {}
var expect = chai.expect

let bounds = boundingBoxOf
let vector = vector3d
  ...
  describe('a nested sphere being reparented under another entity it sits on top of, but keeping original world representation', () => {
    ...
    xdescribe("how reparenting doesn't work using the obvious direct method, as well as being subject to aframe invisibility bug", () => {
      ...
      let reparentDirectly = function(done) {
        ...
        expect(child.object3D.matrixWorld).to.shallowDeepEqual(childWorldMatrix)
        child.object3D.matrixAutoUpdate = false
        child.flushToDOM()
        ...
        child.addEventListener('loaded', () => {
          expect(child.object3D.matrixWorld).to.shallowDeepEqual(childWorldMatrix)
          expect(child.object3D.parent).to.equal(targetParent.object3D)
          done()
        })
        return child
      }
